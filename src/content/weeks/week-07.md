---
week: 7
slug: bounded-agency
title: Bounded Agency
summary: 'Self-fulfilling beliefs, belief webs, boundaries and imprecise beliefs: active inference put
  into an agent-foundations setting.'
prerequisites: ''
objectives: ''
ngoIntro: |
  This week brings together ideas from the last two weeks, in particular by putting active-inference-related ideas into a more agent-foundations setting. See Abram's FixedDT post for a discussion of self-fulfilling beliefs. Then read my belief webs post for some pointers towards how the FixDT framework might be improved.

  Two more ideas which seem very important for extending this line of thinking are boundaries and imprecise beliefs, as discussed in the latter two readings.
readings:
- id: fixdt
  tier: main
  title: FixDT
  authors: Abram Demski
  type: post
  minutes: 30
  prerequisites: |
    Week 3's decision theory. Fixed points of well-behaved maps on compact convex sets (Brouwer, Kakutani).
  year: 2024
  url: https://www.alignmentforum.org/posts/8dbimB7EJXuYxmteW/fixdt
  note: The source doc writes this as "FixedDT"; the post itself is titled FixDT.
- id: agents-as-webs-of-beliefs
  tier: main
  title: Agents as webs of beliefs
  authors: Richard Ngo
  type: post
  minutes: 30
  prerequisites: |
    FixDT, the post above.
  url: https://www.mindthefuture.info/p/agents-as-webs-of-beliefs
- id: boundaries-part-1-a-key-missing-concept-from-utility-theory
  tier: main
  title: 'Boundaries, Part 1: a key missing concept from utility theory'
  authors: Andrew Critch
  type: post
  minutes: 25
  prerequisites: ''
  year: 2022
  url: https://www.lesswrong.com/posts/8oMF8Lv5jiGaQSFvo/boundaries-part-1-a-key-missing-concept-from-utility-theory
- id: imprecise-beliefs-a-tiny-introduction
  tier: main
  title: 'Imprecise beliefs: a tiny introduction'
  authors: davidad
  type: post
  minutes: 10
  prerequisites: |
    Sets of probability distributions in place of a single one.
  url: https://www.lesswrong.com/posts/e7Pd4Q9TF7jFdmPgz/imprecise-beliefs-a-tiny-introduction
- id: explaining-knightianism-on-one-foot
  tier: supplementary
  title: Explaining Knightianism on one foot
  authors: Richard Ngo
  type: post
  minutes: 10
  prerequisites: ''
  url: https://www.mindthefuture.info/p/explaining-knightianism-on-one-foot
- id: selection-vs-control
  tier: supplementary
  title: Selection vs control
  authors: Abram Demski
  type: post
  minutes: 25
  prerequisites: ''
  year: 2019
  url: https://www.lesswrong.com/posts/ZDZmopKquzHYPRNxq/selection-vs-control
- id: introduction-to-cartesian-frames
  tier: supplementary
  title: Introduction to Cartesian Frames
  authors: Scott Garrabrant
  type: post
  minutes: 45
  prerequisites: |
    Sets and functions. A tolerance for unfamiliar formalism.
  year: 2020
  url: https://www.lesswrong.com/posts/BSpdshJWGAW6TuNzZ/introduction-to-cartesian-frames
---

Your own framing for this week goes here. This file's body is rendered as
Markdown below Ngo's introduction, under the heading "Notes for this cohort".
Delete this placeholder text once you have written your own.
