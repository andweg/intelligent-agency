---
week: 2
slug: paradigms-of-cognition
title: Paradigms of Cognition
summary: How probability and computation were formalised, the social setting science grew out of, and
  alignment as an attempt at a new paradigm for cognition.
prerequisites: |
  Fluency with formal definitions. Turing machines.
objectives: ''
ngoIntro: |
  The first two readings this week explore how we gained a formal understanding of two concepts that are crucial for understanding intelligence: probability and computation.

  The third reading focuses on the social milieu in which science began. The final reading from Yudkowsky conveys a sense of alignment research as an attempt to develop a new paradigm for understanding cognition.
readings:
- id: the-emergence-of-probability
  tier: main
  title: The Emergence of Probability
  authors: Ian Hacking
  type: book
  minutes: 75
  prerequisites: ''
  year: 1975
  url: https://www.andreasaltelli.eu/file/repository/Jan_Hacking_Emergence_Probability.pdf#page=81
  excerpt: Chapter 6 (The First Calculations), Chapter 8 (The Great Decision), Chapter 10 (Probability
    and the Law)
- id: the-nature-of-computation-the-grand-unified-theory-of-computation
  tier: main
  title: 'The Nature of Computation: the Grand Unified Theory of Computation'
  authors: Cristopher Moore and Stephan Mertens
  type: book
  minutes: 45
  prerequisites: |
    Fluency with formal definitions. Turing machines.
  year: 2011
  url: https://www.richardcngo.com/the-nature-of-computation-excerpt
  excerpt: Sections 7, 7.1 and 7.2
- id: summary-a-social-history-of-truth
  tier: main
  title: 'Summary: a social history of truth'
  authors: Vaniver
  type: post
  minutes: 20
  prerequisites: ''
  url: https://www.lesswrong.com/posts/qGNXJ3ZGpKWEcfoMj/a-social-history-of-truth
- id: the-rocket-alignment-problem
  tier: main
  title: The Rocket Alignment Problem
  authors: Eliezer Yudkowsky
  type: post
  minutes: 25
  prerequisites: ''
  year: 2018
  url: https://intelligence.org/2018/10/03/rocket-alignment/
- id: pearlian-inference
  tier: supplementary
  title: Pearlian inference
  authors: Judea Pearl
  type: book
  minutes: 90
  prerequisites: ''
  url: https://www.datascienceassn.org/sites/default/files/CAUSAL%2520INFERENCE%2520IN%2520STATISTICS.pdf
  note: 'Linked as Causal Inference in Statistics: A Primer. A commenter suggests The Book of Why instead.'
- id: quantum-computing-since-democritus
  tier: supplementary
  title: Quantum Computing since Democritus
  authors: Scott Aaronson
  type: book
  minutes: 40
  prerequisites: ''
  year: 2013
  url: https://www.are.na/block/1000428
  excerpt: 'Chapter 3: Godel, Turing, and Friends'
---

Your own framing for this week goes here. This file's body is rendered as
Markdown below Ngo's introduction, under the heading "Notes for this cohort".
Delete this placeholder text once you have written your own.
