---
week: 3
slug: expected-utility-maximization
title: Expected Utility Maximization and its Discontents
summary: The axioms of expected utility theory are beautiful, and also flawed. Both perspectives, in one
  week.
prerequisites: ''
objectives: ''
ngoIntro: |
  The axioms of expected utility theory are beautiful, and also flawed. This week tries to explain both perspectives.
readings:
- id: theory-of-games-and-economic-behavior
  tier: main
  title: Theory of Games and Economic Behavior
  authors: John von Neumann and Oskar Morgenstern
  type: book
  minutes: 50
  prerequisites: ''
  year: 1944
  url: https://dn760109.eu.archive.org/0/items/in.ernet.dli.2015.215284/2015.215284.Theory-Of.pdf#page=41
  excerpt: Sections 3.3-3.7
- id: complete-class-consequentialist-foundations
  tier: main
  title: 'Complete class: consequentialist foundations'
  authors: Abram Demski
  type: post
  minutes: 25
  prerequisites: |
    Assumes the vNM axioms from the previous reading, plus the statistical notions of dominance and admissibility.
  year: 2021
  url: https://www.lesswrong.com/posts/sZuw6SGfmZHvcAAEP/complete-class-consequentialist-foundations
- id: geometric-rationality-is-not-vnm-rational
  tier: main
  title: Geometric rationality is not VNM rational
  authors: Scott Garrabrant
  type: post
  minutes: 12
  prerequisites: |
    Only lands against the vNM axioms. Read the von Neumann and Morgenstern excerpt first.
  year: 2022
  url: https://www.lesswrong.com/posts/Xht9swezkGZLAxBrd/geometric-rationality-is-not-vnm-rational
  note: Paired with the next reading as a single bullet in the source doc.
- id: on-the-independence-axiom
  tier: main
  title: On the independence axiom
  authors: Kendiukhov
  type: post
  minutes: 12
  prerequisites: |
    Independence is the axiom stated in §3.6 of the first reading.
  url: https://www.lesswrong.com/posts/MsjWPWjAerDtiQ3Do/on-the-independence-axiom
- id: embedded-agency
  tier: main
  title: Embedded Agency
  authors: Scott Garrabrant and Abram Demski
  type: post
  minutes: 60
  prerequisites: |
    Assumes utility functions, Bayesian updating and the decision-theoretic vocabulary of everything above it.
  year: 2018
  url: https://www.lesswrong.com/s/Rm6oQRJJmhGCcLvxh/p/i3BTagvt3HbPMx6PN
  excerpt: Sections 1 (Embedded agents), 2 (Decision theory) and 3 (Embedded world-models)
- id: harsanyi-s-utilitarian-theorem-without-tears
  tier: supplementary
  title: Harsanyi's utilitarian theorem without tears
  authors: Johan Gustafsson and Kowalczyk
  type: paper
  minutes: 30
  prerequisites: |
    Assumes the vNM axioms.
  url: https://www.utilitarianism.net/img/Harsanyi-Utilitarian-Theorems-without-Tears.pdf
- id: futarchy
  tier: supplementary
  title: Futarchy
  authors: Robin Hanson
  type: post
  minutes: 25
  prerequisites: ''
  url: https://mason.gmu.edu/~rhanson/futarchy.html
- id: toward-negotiable-reinforcement-learning
  tier: supplementary
  title: Toward negotiable reinforcement learning
  authors: Andrew Critch
  type: paper
  minutes: 35
  prerequisites: |
    Assumes the reinforcement learning formalism and vNM utility.
  year: 2017
  url: https://arxiv.org/pdf/1701.01302
- id: servant-of-many-masters
  tier: supplementary
  title: Servant of many masters
  authors: Andrew Critch and Stuart Russell
  type: paper
  minutes: 35
  prerequisites: ''
  year: 2017
  url: https://arxiv.org/abs/1711.00363
- id: utility-maximization-description-length-minimization
  tier: supplementary
  title: Utility Maximization = Description Length Minimization
  authors: John Wentworth
  type: post
  minutes: 15
  prerequisites: ''
  year: 2021
  url: https://www.lesswrong.com/s/ogntdnjG6Y9tbLsNS/p/voLHQgNncnjjgAPH7
- id: picoeconomics-in-a-nutshell
  tier: supplementary
  title: Picoeconomics in a nutshell
  authors: George Ainslie
  type: post
  minutes: 20
  prerequisites: ''
  url: https://www.picoeconomics.org/Nut2/Nutshell2.html
---

Your own framing for this week goes here. This file's body is rendered as
Markdown below Ngo's introduction, under the heading "Notes for this cohort".
Delete this placeholder text once you have written your own.
