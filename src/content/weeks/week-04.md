---
week: 4
slug: on-generalization
title: On Generalization
summary: Generalization in ML, and how it clashes with the rational-agent approach taken in alignment.
prerequisites: |
  Policy-gradient reinforcement learning: reward as something that shapes the policy through gradient updates, not something the policy seeks. Compression and Kolmogorov complexity, informally: the shortest program that outputs the data. The formal RL setting — Markov decision processes, return, the reward hypothesis — for one supplementary reading.
objectives: ''
ngoIntro: |
  This week focuses on the concept of generalization in ML, and how it clashes with the rational-agent approach taken in alignment. Note that I'm currently writing up a more detailed history of the engagement between the ML ontology and the alignment ontology, which will replace my two pieces here.
readings:
- id: agi-safety-from-first-principles-superintelligence
  tier: main
  title: 'AGI safety from first principles: Superintelligence'
  authors: Richard Ngo
  type: post
  minutes: 15
  prerequisites: ''
  year: 2020
  url: https://www.alignmentforum.org/s/mzgtmmTKKn5MuCzFJ/p/eG3WhHS8CLNxuH6rT
  note: Paired with the next reading as a single bullet in the source doc.
- id: the-ml-ontology-and-the-alignment-ontology
  tier: main
  title: The ML ontology and the alignment ontology
  authors: Richard Ngo
  type: post
  minutes: 15
  prerequisites: ''
  url: https://www.lesswrong.com/posts/Yz4YHncz2vwN4ksDA/the-ml-ontology-and-the-alignment-ontology
- id: an-observation-on-generalization
  tier: main
  title: An Observation on Generalization
  authors: Ilya Sutskever
  type: video
  minutes: 36
  prerequisites: |
    Compression and Kolmogorov complexity, informally: the shortest program that outputs the data.
  year: 2023
  url: https://www.youtube.com/live/AKMuA_TVz3A
  excerpt: Until 36:00
- id: situational-awareness-makes-measuring-safety-tricky
  tier: main
  title: '"Situational awareness" makes measuring safety tricky'
  authors: Ajeya Cotra
  type: video
  minutes: 30
  prerequisites: ''
  url: https://www.youtube.com/watch?v=DFtnu0Qn1VM
- id: reward-is-not-the-optimization-target
  tier: main
  title: Reward is not the optimization target
  authors: Alex Turner
  type: post
  minutes: 20
  prerequisites: |
    Policy-gradient reinforcement learning: reward as something that shapes the policy through gradient updates, not something the policy seeks.
  year: 2022
  url: https://www.lesswrong.com/posts/pdaGN6pQyQarFHXF4/reward-is-not-the-optimization-target
- id: settling-the-reward-hypothesis
  tier: supplementary
  title: Settling the reward hypothesis
  authors: Michael Bowling et al.
  type: paper
  minutes: 45
  prerequisites: |
    The formal RL setting: Markov decision processes, return, the reward hypothesis.
  year: 2022
  url: https://arxiv.org/pdf/2212.10420
  note: Listed under "Extra readings" in the source doc.
- id: goal-misgeneralization
  tier: supplementary
  title: Goal misgeneralization
  authors: Rohin Shah et al.
  type: paper
  minutes: 45
  prerequisites: ''
  year: 2022
  url: https://arxiv.org/abs/2210.01790
- id: the-alignment-problem-from-a-deep-learning-perspective
  tier: supplementary
  title: The alignment problem from a deep learning perspective
  authors: Richard Ngo, Lawrence Chan and Soren Mindermann
  type: paper
  minutes: 50
  prerequisites: ''
  year: 2022
  url: https://arxiv.org/abs/2209.00626
---

Your own framing for this week goes here. This file's body is rendered as
Markdown below Ngo's introduction, under the heading "Notes for this cohort".
Delete this placeholder text once you have written your own.
